...
module mac_pe #(
  ...
  input  wire                  valid_in,
  input  wire                  last_in,    // pulse high on the 9th (final) input
  input  wire [DATA_WIDTH-1:0] pixel,
  input  wire [DATA_WIDTH-1:0] weight,
  output reg  [ACC_WIDTH-1:0]  acc_out,
  output reg                   valid_out   // only pulses after the final accumulation
);

  // ---- Stage 1: Register inputs ----
  reg [DATA_WIDTH-1:0] s1_pixel, s1_weight;
  reg                  s1_valid, s1_flush, s1_last;
  ...
  // ---- Stage 3: Accumulate ----
  // valid_out only fires when s2_last is set — i.e. the final tap has been accumulated
  always @(posedge clk) begin
    if (!rst_n) begin
      acc_out   <= 0;
      valid_out <= 0;
    end else begin
      valid_out <= s2_valid && s2_last;   // only pulse on final accumulation
      if (s2_flush)
